...
echo "Updating [${fname}]"
curl -s https://raw.githubusercontent.com/Homebrew/homebrew-core/master/Formula/m/maven.rb | perl -pe 's/class Maven < Formula/class MavenNoopenjdk < Formula/g; s/depends_on \"openjdk\"//g' | perl -0pe 's/bottle do.*?end//gms' > ${fname}
